...
// The folders we just copied to dist
const videoMappings = {
    "1 Mah": { dir: "1 Mah V", file: "" }, // Still missing
...
// We need to parse the data.js content. It starts with const apartmentsData = [ ... ];
let jsonStr = dataStr.replace('const apartmentsData = ', '').replace(/;\s*$/, '');
let apartments = JSON.parse(jsonStr);

apartments.forEach(apt => {
    const mapping = videoMappings[apt.id];
    if (mapping) {
        apt.videoDir = mapping.dir;
        apt.video = mapping.file;
    }
});

fs.writeFileSync(dataPath, `const apartmentsData = ${JSON.stringify(apartments, null, 2)};\n`, 'utf8');
console.log("Updated data.js with actual video paths.");
