...
const dir = 'c:\\\\Users\\\\Александр\\\\OneDrive\\\\Рабочий стол\\\\alanya rent website\\\\11 Mah';
const infraDir = path.join(dir, 'Infrustructure');

let files = fs.readdirSync(dir).filter(f => f.match(/\.(jpg|jpeg|png)$/i));
if (fs.existsSync(infraDir)) {
    const infraFiles = fs.readdirSync(infraDir).filter(f => f.match(/\.(jpg|jpeg|png)$/i)).map(f => 'Infrustructure/' + f);
    files = files.concat(infraFiles);
}
