...
for (let i = 0; i < lines.length; i++) {
   if (lines[i].includes('"id": "10 Mah"')) {
       in10mah = true;
   }
   if (in10mah && lines[i].includes('"size": ')) {
       in10mah = false; // logic exit
   }
   if (in10mah && lines[i].includes(`"en": "<div class=\\"premium-desc pd-v3\\">`)) {
       let start = i;
       let end = i;
...
console.log('Successfully updated 10 Mah en richDescription');
