...
for (let apt of data) {
    if (apt.id === '11 Mah') {
        apt.title = {
            ru: "Tekinoglu 1+1 (Вид на море)",
            en: "Tekinoglu 1+1 (Sea View)",
            tr: "Tekinoglu 1+1 (Deniz Manzaralı)",
            de: "Tekinoglu 1+1 (Meerblick)"
        };
        apt.description = {
            ru: "Эту квартиру выбирают те, кто хочет встречать рассветы, наблюдая как солнечные лучи купаются в море.",
            en: "Chosen by those who want to greet the dawn watching the sunrays bathe in the sea.",
            tr: "Güneş ışınlarının denizde yıkanmasını izleyerek şafağı karşılamak isteyenlerin tercihi.",
            de: "Ausgewählt von jenen, die den Sonnenaufgang begrüßen möchten, während die Sonnenstrahlen im Meer baden."
        };
        apt.size = "1+1, 50m²";
        ...
        apt.videoDir = '11 Mah v';
        apt.video = 'IMG_2766.mov';
        
        console.log('Updated 11 Mah data successfully!');
        found = true;
    }
}

if (!found) {
    console.log('Could not find 11 Mah in data.js');
}
